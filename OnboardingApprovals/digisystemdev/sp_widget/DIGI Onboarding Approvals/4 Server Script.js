...
//var qc1 = gr.addQuery("state", "requested"); Filtro padrão
var qc1 = gr.addEncodedQuery(options.state_filter);
data.myApprovals = getMyApprovals();
gr.addQuery("approver", data.myApprovals);
gr.addEncodedQuery(options.query_filter);

//gr.orderBy("sys_created_on"); Filtro padrão
...
while (gr.next()) {
  var task = getRecordBeingApproved(gr); // Task
  ...
  t.short_description = task.short_description.toString(); // variables.u_job_title
  t.u_job_vacancy_name = task.variables.u_job_vacancy_name.getDisplayValue();

  var dateTime = new GlideDateTime(task.opened_at.getDisplayValue().toString())
  var dt = dateTime.getDate()
  t.opened_at = dt.getByFormat("dd/MM/YY");
 

  //t.short_description = task.variable.u_job_title.toString();
